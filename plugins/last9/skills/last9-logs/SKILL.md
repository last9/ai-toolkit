...
|------|---------|-----------|
| `get_log_attributes` | Discover available log/resource field names | time params; optional `index` |
...
| 6 | Assuming an attribute name carries over between services (`attributes['http.status_code']` on one service, `attributes['status_code']` on another) | Attribute names vary per service with no error on a wrong guess — the filter silently matches nothing. Verify the name via `get_log_attributes` before filtering on it |
...
Include `index` in `get_logs` / `get_log_attributes` **only when the user explicitly names one**. Never guess or invent an index — omit the parameter entirely otherwise.
