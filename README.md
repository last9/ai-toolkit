...
The top-level `skills/` directory is canonical; the packaged copies under `plugins/last9/skills/` and `plugins/opencode-last9/skills/` are generated by `scripts/sync-agent-plugin-skills.sh`, and CI fails any PR where they drift. See [CONTRIBUTING.md](CONTRIBUTING.md) for the workflow, and [SECURITY.md](SECURITY.md) for reporting vulnerabilities.
